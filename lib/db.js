...
export async function incrementCookedCount(id) {
  ...
  const { error } = await supabase
    .from('recipes').update({ cooked_count: (current.cooked_count || 0) + 1 }).eq('id', id);
  if (error) throw error;
}
...
export async function addToShoppingList(recipeId, recipeName, ingredients) {
  const { error } = await supabase.from('shopping_list').insert(
    ingredients.map(ing => ({ recipe_id: recipeId, recipe_name: recipeName, ingredient_name: ing.name, qty: ing.qty || '' }))
  );
  if (error) throw error;
}
