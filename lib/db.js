...
export async function saveProfile(userId, { firstName, lastName, username, unitPreference }) {
  const { data, error } = await supabase
    .from('profiles')
    .upsert({
      id: userId,
...
export async function getUserPublicProfile(targetUserId) {
  const user = await getUser();
  const [{ data: profile }, cookbooks, followCounts, isFollowing] = await Promise.all([
  ...
  return { profile, cookbooks, followCounts, isFollowing };
}
